...
resource "aws_ec2_transit_gateway_route_table_association" "default" {
  for_each                       = { for tgwra in local.transit_gateway_route_table_association : tgwra.identifier => tgwra }
  transit_gateway_attachment_id  = local.tgwa_ids[each.value.tgw_attachment_identifier]
  transit_gateway_route_table_id = local.tgwr_ids[each.value.tgw_route_table_identifier]
...
resource "aws_ec2_transit_gateway_route_table_propagation" "default" {
  for_each                       = { for tgwra in local.transit_gateway_route_table_propagation : tgwra.identifier => tgwra }
  transit_gateway_attachment_id  = local.tgwa_ids[each.value.tgw_attachment_identifier]
  transit_gateway_route_table_id = local.tgwr_ids[each.value.tgw_route_table_identifier]
}
